<template>
    <view class="followup-container">
      ...
      <!-- 添加跟进记录表单 -->
      <view class="add-followup-form"  v-if="type == 'loan'">
        <view class="form-title">添加详细进度</view>
        <view class="form-item">
          <view class="form-item-content">
            <textarea v-model="formData.content" placeholder="请输入详细进度" />
          </view> 
        </view>
 ...
 import financeApi from '@/api/finance.js';
  ...
  export default {
    ...
    onLoad(options) {
        ...
        if (options.loanData) {
          try {
            const loanData = JSON.parse(decodeURIComponent(options.loanData));
    ...
    methods: {
      ...
      loadFollowupList() {
        ...
        // 调用API获取跟进记录
        financeApi.getLoanDealLog({ id: this.loanId }).then(res => {
          this.loading = false;
